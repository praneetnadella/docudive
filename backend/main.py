...
def multi_doc_retrieval(sorted_matches):
    ...
    for match in sorted_matches:
        ...
        doc_groups.setdefault(fname, []).append(match)
    final = []
    for fname, matches in doc_groups.items():
        matches.sort(key=lambda x: x.get("score", 0), reverse=True)
        final.extend(matches[:3])
...
def advanced_retrieval(query_embedding, classification: str, score_threshold=0.2):
    raw_matches = query_pinecone(query_embedding, top_k=40)
    if not raw_matches:
        return []
...
async def query_document(question: str = Form(...)):
    ...
    for idx, c in enumerate(chunks, start=1):
        ...
        context_lines.append(f"[{idx}] Source: {c['text']}")
    context_text = "\n\n".join(context_lines)
    
    doc_prompt = f"""You are an expert assistant with full access to the uploaded document excerpts.
        Answer the following question based solely on the context provided below.
        Do not add any information beyond the provided excerpts, and do not state that the document lacks details if they are present.
        Include inline citations in the format [1], [2], etc. to reference the source excerpts.
